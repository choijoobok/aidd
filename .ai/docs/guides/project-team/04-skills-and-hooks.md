...
훅이 없거나 비활성화된 환경에서도 정상 용어 변경은 `term-review`와 `term-apply`를 사용한다. `term-apply`가 생성·검증을 자체 완료하므로 별도 승인 상태나 전체 테스트를 추가하지 않는다.

요구분석·설계 중 새 용어를 알아채고 PM에게 확인하는 동작은 대화 의미를 판단하는 `aidd-discovery`와 `aidd-architecture` 스킬의 책임이다. 훅은 사용자 의도를 이해하거나 승인 대화를 수행하지 않으므로 이 자동화를 훅으로 구현하지 않는다. PM이 확인하면 AI가 같은 작업 흐름에서 `term-apply`까지 실행하며, 사용자가 명령을 따로 입력할 필요는 없다.

...
- 훅 신뢰 리비전은 실제 Codex provider 정의 `.codex/hooks.json`만 해시한다. `/hooks`에 새 정의가 나타나는 이 파일의 내용 변경만 재승인을 요구하고, 정의를 바꾸지 않는 정책·실행기·계약·테스트·가이드 수정은 세션을 잠그지 않는다. 기존 상태 파일의 넓은 리비전 범위는 현재 신뢰 정의로 한 번 마이그레이션한다.

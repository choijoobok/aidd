...
프로젝트 용어의 추가·변경·제거 적용은 PM만 확인할 수 있다. 요구분석·설계 중 AI가 의미·명명·계약에 영향을 주는 새롭거나 모호한 용어를 발견하면 기존 용어를 먼저 검색한다. 모든 활성 팀원은 읽기 전용 `term-review`로 개념 유형·분류·정의·범위·예시·관련 용어·혼동 구분과 영향 후보를 담은 확인 카드를 만들 수 있다. 오프라인 협업에서는 팀원이 결과를 복사해 PM에게 전달하고, PM이 확인한 카드와 승인 의사를 자신의 AI 세션에 전달한다. 그 뒤에만 AI가 관련 정본·문서·소스 주석을 현행화하고 PM 신원으로 `term-apply`를 실행해 현재 `TRM`, `TCH` 이력, 파생 문서 생성과 검증을 한 흐름으로 완료한다. 일회성 표현은 등록하지 않고 팀원 제안·승인 대기·반려 상태도 별도로 만들지 않는다. 전체 생성의 내부 통합 용어집은 `project/docs/generated/site/operations/index.html`, 최종 사용자 공개 용어 뷰는 `project/docs/generated/site/user/index.html`에 포함된다. `delivery-glossary`는 DLP 독자 설정에 따라 독립 오프라인 용어집 패키지를 조립하며, 최종 사용자용 결과에는 고객 공개 최종 사용자 용어만 포함하고 source metadata는 포함하지 않는다.

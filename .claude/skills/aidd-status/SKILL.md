...
7. 병합 후 재검토는 PM·관리자의 오프라인 지시 대상이므로 대기 `MRC`에 사전 담당자를 요구하지 않는다. 완료된 `MRC`만 실제 수행자·시각·결과·증거를 표시한다. 파일 동등성 검증을 AI 행동 동등성으로, 로컬 ruleset을 원격 보호 활성화로 표현하지 않는다. 각 실행 슬롯과 원격 검증의 실제 상태를 별도로 표시한다.

...
| 결정 질문 저장·세션 재개·답변 반영 | `aidd-decision-management` |
...
> 이전 세션에서 답하지 않은 결정을 정본에서 찾아 같은 질문부터 이어가고, 답변을 관련 정본과 결정 이력에 반영해줘.
...
- `UserPromptSubmit`에서 다음 응답에 속한 사용자 원문을 모두 임시 보관하고 `Stop`에서 최종 응답과 결합해 `Codex` 또는 `Claude`가 표시된 한 블록을 `chat-history/YYYY-MM/raw/YYYY-MM-DD.md`에 잠금 아래 한 번에 추가; Codex는 처리 중 추가 질의도 세션 큐에 순서대로 보존하고 transcript에서 사용자에게 표시된 진행 메시지만 최선 노력으로 `AI PROGRESS`에 포함하며 추론·도구 호출·도구 출력·소스 diff는 제외함; 한쪽만 있는 블록은 기록하지 않음

...
- Codex Desktop에서 프로젝트 훅의 검토·신뢰 화면이 나타나지 않은 경우, 세션 시작 AI는 `python .ai/tools/aidd.py hook-trust-status`로 신뢰 기록 부재를 알린다. 이 명령은 훅을 우회 실행하거나 신뢰 상태를 바꾸지 않는다. 사용자는 터미널에서 `codex -C "<프로젝트 루트>"`를 열어 `/hooks`에서 `UserPromptSubmit`과 `Stop` 훅을 직접 검토·신뢰한다.

...
제품 코드를 변경하기 전 CHG의 `delivery_path`와 `.aidd/ssot/system-surfaces/MOD-ID.json`의 `SURF`를 확인한다. 신규 기능은 분석·설계·문서를 먼저 작성한다. 기존 문서가 있는 화면·API 등은 코드와 같은 변경에서 현행화하고, 문서가 없는 기존 기능을 나중에 작성하기로 했다면 대상 문서와 완료 마일스톤 또는 릴리스가 있는 `documentation-reconciliation` WRK를 만든다. 레거시 고도화는 `aidd-legacy-reconciliation`으로 전체 표면과 기존 문서를 조사한 뒤 단계별 계획을 세운다. 커밋 전 훅이 staged 소스와 이 연결을 검사한다.

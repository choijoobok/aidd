...
import { bumpVersion, prepareRelease, releaseMetadataErrors, releasePlan } from "../tools/kit.mjs";
...
test("prepare release updates every release authority without creating Git state",()=>{
  ...
  try{
    const plan=prepareRelease({root,date:"2026-09-22",requireClean:false});
...
test("prepare release refuses unsafe or ambiguous state before writing",()=>{
  ...
  try{
    ...
    assert.throws(()=>prepareRelease({root,date:"2026-09-22",requireClean:false}),/KIT-CHG-002: status is in_review/);
    ...
    assert.throws(()=>prepareRelease({root,date:"2026-09-22",requireClean:false}),/target release record already exists/);
    assert.equal(readFileSync(join(root,".aidd-kit-dev/repository.json"),"utf8"),before);

...
### MLS-001 — 실행 가능한 AIDD 사전 기반 완성

- **id:** MLS-001
- **title:** 실행 가능한 AIDD 사전 기반 완성
- **status:** in_progress
...
- **status:** current
- **provider:** MOD-GOV
- **consumers:** MOD-DOC, MOD-STATUS, MOD-QA
- **contract:** FILES 레지스트리와 render_documents가 모든 정본 및 파생 문서를 결정적으로 연결한다.
- **compatibility:** 필드 변경 시 생성기·검증·테스트·산출물 목록을 같은 변경에서 갱신한다.
- **requirements:** REQ-001, REQ-002, REQ-003

### IFC-002 — 공통 AI 스킬-플랫폼 어댑터 계약

- **id:** IFC-002
- **name:** 공통 AI 스킬-플랫폼 어댑터 계약
- **status:** current
- **provider:** MOD-AI
- **consumers:** MOD-DISC, MOD-ARCH, MOD-DELIVERY, MOD-QA, MOD-STATUS
- **contract:** AGENTS.md 공통 계약은 Codex가 직접 읽고 Claude Code가 CLAUDE.md에서 import한다. .ai/skills 정본은 두 검색 경로에 동기화하고 동일성을 검사한다.
- **compatibility:** 플랫폼 전용 지침과 훅은 얇게 유지하고 공통 수행 규칙은 AGENTS.md, 공통 스킬 의미는 .ai/skills에 둔다.
- **requirements:** REQ-013, REQ-026

### IFC-003 — CI-브랜치 보호 계약
...
- **provider:** MOD-QA
- **consumers:** MOD-CHG
- **contract:** 브랜치 보호의 필수 검사 이름은 GitHub Actions 작업 이름과 정확히 일치한다.
- **compatibility:** 작업 이름 변경 시 repository.json과 ruleset을 함께 변경한다.
- **requirements:** REQ-023, REQ-027

### IFC-004 — 개발 기반 정본-파생 산출물 계약

- **id:** IFC-004
- **name:** 개발 기반 정본-파생 산출물 계약
- **status:** current
- **provider:** MOD-ARCH
- **consumers:** MOD-DOC, MOD-DELIVERY, MOD-QA
- **contract:** STD·GPH·EXC·UXB·UIP·CMP와 모듈별 SCR·MAN, RUN·DLP 정본을 안정 ID로 연결하고 문서·목업·manifest를 결정적으로 생성한다.
- **compatibility:** 필드나 상태 전이를 바꾸면 로더·검증기·생성기·산출물 카탈로그와 TC-024·TC-025를 같은 변경에서 갱신한다.
...
- **type:** contract
- **status:** current
- **description:** 정본 구조가 생성 문서의 입력 계약을 제공한다.
- **verification:** TC-002와 TC-016
...
- **type:** contract
- **status:** current
- **description:** 상태 뷰는 정본 ID와 증거 상태만 집계한다.
- **verification:** TC-001과 TC-015
...
- **type:** workflow
- **status:** blocked
- **description:** 실제 교차 평가에는 검증된 공통 하네스가 선행한다.
- **verification:** TC-017과 TC-018
...
- **type:** workflow
- **status:** blocked
- **description:** 원격 보호 활성화에는 로컬 규칙 구성과 GitHub Pro·공개 저장소·대체 호스팅 중 하나의 정책 결정이 필요하다.
- **verification:** TC-019와 TC-020
...
- **type:** contract
- **status:** current
- **description:** 개발 기반 정본 계약이 생성기·검증기와 프로젝트별 UI·운영 파생물 구현의 입력이다.
- **verification:** TC-024와 TC-025

...
test("PM reviews then atomically applies terminology while team members cannot apply it",()=>{
  ...
  try{
    ...
    const terminologyPath=join(workspace,"project/.aidd/ssot/terminology.json"),beforeReview=readFileSync(terminologyPath,"utf8"),definition="고객이 지원이나 처리를 요청하는 업무 단위다.";
    const review=run(workspace,["term-review","--action","add","--id","TRM-002","--term","지원 요청","--key","supportRequest","--category","업무","--definition",definition,"--requested-by","HUM-001","--audience","project_team"]);
    ...
    const denied=run(workspace,["term-apply","--action","add","--id","TRM-002","--history","TCH-002","--term","지원 요청","--key","supportRequest","--category","업무","--definition",definition,"--approved-by","HUM-002","--summary","팀원 적용 시도"]);
    ...
    const added=run(workspace,["term-apply","--action","add","--id","TRM-002","--history","TCH-002","--term","지원 요청","--key","supportRequest","--category","업무","--definition",definition,"--approved-by","HUM-001","--summary","PM 영향 검토 후 용어 추가","--audience","project_team"]);

...
- 승인 전 shell 읽기 예외는 합성·파이프·개행·리다이렉션·명령 치환이 없는 단일 명령으로 제한하고, 읽기 명령의 쓰기 가능한 옵션도 차단한다. Windows package 또는 명시적인 Desktop origin은 앱으로, origin과 package가 모두 없는 실제 CLI 환경은 CLI로 식별하며 그 밖의 알 수 없는 origin에는 더 제한적인 새 세션 확인 절차를 적용한다. 최초 `unconfirmed`, 훅 리비전이 오래된 세션과 `restart_required` 상태는 모두 같은 읽기·훅 유지보수·관측 경로 close-out 예외를 사용한다.

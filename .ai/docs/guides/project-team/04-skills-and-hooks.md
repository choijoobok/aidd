...
| 승인 전 HTML 목업 | `aidd-ui-prototype` |
...
> 설계 중 새 용어가 나왔어. 기존 용어와 비교하고, 필요하면 PM에게 전달할 용어 확인 카드를 만들어줘. 아직 용어집은 바꾸지 마.
...
- 프로젝트 용어 정본을 직접 변경한 경우 용어 문서 현행화 보조; 정상 PM 흐름의 `term-apply`는 자체적으로 생성·검증까지 완료
...
훅이 없거나 비활성화된 환경에서도 정상 용어 변경은 `term-review`와 `term-apply`를 사용한다. `term-review`는 모든 활성 팀원이 PM 전달용 확인 카드를 만드는 읽기 전용 명령이고, `term-apply`만 승인 PM의 현재 Git 신원을 요구한다. `term-apply`가 생성·검증을 자체 완료하므로 별도 승인 상태나 전체 테스트를 추가하지 않는다.

요구분석·설계·구현 중 새롭거나 모호한 용어를 발견하고 확인 카드부터 PM 적용까지 조율하는 책임은 `aidd-terminology` 스킬에 있다. `aidd-discovery`, `aidd-architecture`, `aidd-delivery`는 용어 판단이 필요할 때 이 스킬로 연결한다. 훅은 사용자 의도를 이해하거나 승인 대화를 수행하지 않으므로 이 자동화를 훅으로 구현하지 않는다. 팀원과 PM이 다른 세션을 사용하면 팀원이 카드 결과를 복사해 오프라인으로 전달하고, PM이 확인한 카드와 승인 의사를 AI에게 전달한다. PM 확인 뒤 AI가 `term-apply`까지 실행하므로 PM이 명령을 직접 입력할 필요는 없다.

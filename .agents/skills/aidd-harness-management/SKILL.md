...
5. 프로젝트에서는 변경 후 `node .ai/tools/aidd.mjs sync-ai`, `node .ai/tools/aidd_hook.mjs self-test --hook`, 관련 행위 테스트를 실행한다. Kit 원본의 provider 동기화와 릴리스 검증은 배포되지 않는 관리자 가이드를 따른다. provider별 별도 평가나 기록은 사용자가 요청하거나 실제 provider 차이를 재현할 때만 만든다.

      ...
      withLock(path,()=>{
        ...
        if(existsSync(path)){
          try{
            ...
            else if(String(current.prompt??"").trim())pending.prompts.push({submitted_at:Number(current.started_at)||submittedAt,prompt:String(current.prompt).slice(0,MAX_ITEM_CHARS)});
          }catch{}
        }
        pending.prompts.push({submitted_at:submittedAt,prompt:prompt.slice(0,MAX_ITEM_CHARS)});

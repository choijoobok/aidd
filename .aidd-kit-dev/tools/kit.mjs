...
function git(...args){const result=spawnSync("git",args,{cwd:ROOT,encoding:"utf8"});return result.status===0?result.stdout.trim():null;}
...
function originRecord(stage,manifest){const status=git("status","--porcelain");return{schema_version:1,purpose:"provenance-only",upgrade_contract:"none",kit_version:manifest.kit_version,source_commit:git("rev-parse","HEAD")??"uncommitted",source_dirty:Boolean(status),export_manifest_sha256:sha256(readFileSync(MANIFEST_PATH)),payload_sha256:aggregateHash(fileManifest(stage,new Set([".aidd-kit-origin.json"])))};}
...
const [command,...rest]=process.argv.slice(2),options=parseOptions(rest);
try{
  ...
  else if(["export","new-project"].includes(command)){const directory=options.directory,archive=options.zip;if(!directory&&!archive)throw new Error("--directory or --zip is required");const temp=mkdtempSync(join(tmpdir(),"aidd-kit-export-")),stage=join(temp,"payload");mkdirSync(stage);const project=command==="new-project"?{project_id:options["project-id"],name:options.name,mode:options.mode??"greenfield",source_location:options["source-location"]??""}:null;assemble(stage,project?"product-workspace":"kit-template",project);const destination=deliver(stage,directory,archive);rmSync(temp,{recursive:true,force:true});console.log(`AIDD ${project?"product-workspace":"kit-template"} created: ${destination}`);}

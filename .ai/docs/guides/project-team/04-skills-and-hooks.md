...
요구분석·설계 중 새 용어를 알아채고 확인 카드를 만드는 동작은 대화 의미를 판단하는 `aidd-discovery`와 `aidd-architecture` 스킬의 책임이다. 훅은 사용자 의도를 이해하거나 승인 대화를 수행하지 않으므로 이 자동화를 훅으로 구현하지 않는다. 팀원과 PM이 다른 세션을 사용하면 팀원이 카드 결과를 복사해 오프라인으로 전달하고, PM이 확인한 카드와 승인 의사를 AI에게 전달한다. PM 확인 뒤 AI가 `term-apply`까지 실행하므로 PM이 명령을 직접 입력할 필요는 없다.

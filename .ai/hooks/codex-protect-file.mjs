#!/usr/bin/env node
/** Isolated Codex file protection hook. Policy is intentionally duplicated. */
...
const paths=typeof raw==="string"?(raw.includes("*** Begin Patch")?[...raw.matchAll(/^\*\*\* (?:Add|Update|Delete) File: (.+)$/gm)].map(m=>normalize(m[1])):[normalize(raw)]):written(raw,"",0);

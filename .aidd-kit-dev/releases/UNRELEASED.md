...
- 증거의 `performed_at`은 시스템 시계에서 읽은 값만 허용하며 시각 성분이 있는 미래 값을 거부한다. 예외는 코드 안의 허용 목록(`KIT-EVD-024`~`028`)으로만 적용되며 레코드가 스스로 예외를 부여할 수 없다.
- `KIT-EVD-009`·`KIT-EVD-010`의 이전 형식과 `KIT-EVD-001~008`의 날짜 단위 시각은 소급 수정하지 않고 그대로 보존한다.
- 레코드 탐색은 각 디렉터리의 직속 파일만 대상으로 하고 파일 이름 규칙을 강제한다. `evidence/*-logs/`의 원시 산출물은 검사 대상이 아니다.
- `kit status`가 `active_change` 외에 진행 중인 다른 변경도 함께 출력한다.
...
- 검증 근거: `KIT-EVD-031`, 자기부여 예외·원시 로그 오탐 MEDIUM 2건을 확인한 독립 재검토와 그 수정 `KIT-EVD-032`.

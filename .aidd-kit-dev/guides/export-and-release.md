...
Codex 훅 어댑터의 정본은 `.aidd-kit-dev/export/.codex/hooks.json`이다. Codex 전용 `SessionStart` 훅 신뢰 확인 경고는 이 어댑터에만 연결하고 Claude 어댑터에는 추가하지 않는다. 어댑터를 바꾸면 `sync-providers`로 루트 복사본을 갱신한 뒤 self-test와 smoke로 export 경계를 확인한다.

...
이 기반 구조는 2026-09-17 기준 원저자·표준기관·공식 프로젝트의 1차 자료를 확인하여 설계했다. 방법론을 순위화하거나 하나를 그대로 도입하지 않고, 해결하려는 문제와 한계를 분석하여 AIDD 단계·게이트·증거에 연결했다. 구조화된 비교 결과는 `.aidd/ssot/methodologies.json`과 `docs/generated/methodology-comparison.md`가 정본과 생성 뷰 역할을 맡는다.
...
이 프레임워크는 특정 Java·Spring 기술을 모든 프로젝트에 강제하지 않는다. 위 사례를 프록시 경계, 실행 컨텍스트, 자원 풀과 분산 동시성 같은 일반 설계 위험으로 추상화하여 `.aidd/ssot/deployment.json`의 DPR 패턴으로 관리한다.
